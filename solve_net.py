...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            ...
            loss_list_s.append(np.mean(loss_list))
            acc_list_s.append(np.mean(acc_list))
            loss_list = []
            acc_list = []
            LOG_INFO(msg)

    return np.mean(loss_list_s), np.mean(acc_list_s)
...
def test_net(model, loss, inputs, labels, batch_size):
    ...
    loss_list_s = np.mean(loss_list)
    acc_list_s = np.mean(acc_list)
    LOG_INFO(msg)

    return loss_list_s, acc_list_s
